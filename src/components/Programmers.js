...
export default function Programmers() {
  ...
  const [programmersList, setProgramersList] = useState(listOfAwesome);
  const [featuredProgrammerID, setFeaturedProgramerID] = useState(null)
      ...
      <div className='programmers'>
        {
          ...
          programmersList.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => {featuredProgrammerID !== dev.id ? setFeaturedProgramerID(dev.id) : setFeaturedProgramerID(null);} }>Feature</button>
            </div>
          )
        }
      </div>
